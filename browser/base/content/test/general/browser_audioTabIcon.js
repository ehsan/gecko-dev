...
function* wait_for_tab_playing_event(tab, expectPlaying) {
  yield BrowserTestUtils.waitForEvent(tab, "TabAttrModified", false, (event) => {
    if (event.detail.changed.indexOf("playing") >= 0) {
      is(tab.hasAttribute("playing"), expectPlaying, "The tab should " + (expectPlaying ? "" : "not ") + "be playing");
      return true;
    }
    return false;
  });
...
function* wait_for_tab_muted_event(tab, expectMuted) {
  yield BrowserTestUtils.waitForEvent(tab, "TabAttrModified", false, (event) => {
...
function* test_mute_tab(tab, icon, expectMuted) {
  let playingAudioContextMenu = document.getElementById("playingAudioContextMenu");
  let popupShownPromise = BrowserTestUtils.waitForEvent(playingAudioContextMenu, "popupshown");
  EventUtils.synthesizeMouseAtCenter(icon, {button: 2, type: "contextmenu"});
  yield popupShownPromise;

  let mute = document.getElementById("muteAudio");
  is(mute.label, expectMuted ? "Mute Tab" : "Unmute Tab", "The menu item should have the correct title");
  EventUtils.synthesizeMouseAtCenter(mute, {});
  yield wait_for_tab_muted_event(tab, expectMuted);
}

function* test_playing_icon_on_tab(tab, browser, isPinned) {
  let icon = document.getAnonymousElementByAttribute(tab, "anonid",
                                                     isPinned ? "overlay-icon" : "playing-icon");
