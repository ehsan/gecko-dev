...
#endif
  WMF_LOG("[%p] WMFByteStream CTOR", this);
...
WMFByteStream::~WMFByteStream()
{
  ...
  WMF_LOG("[%p] WMFByteStream DTOR", this);
}
...
WMFByteStream::Init()
{
  ...
  if (!contentTypeUTF16.IsEmpty()) {
    ...
    WMF_LOG("[%p] WMFByteStream has Content-Type=%s", this, mResource->GetContentType().get());
  }
  return NS_OK;
}
...
{
  WMF_LOG("[%p] WMFByteStream::QueryInterface %s", this, GetGUIDName(aIId).get());
...
{
  WMF_LOG("ReadRequest::QueryInterface %s", GetGUIDName(aIId).get());
                         ...
                         IUnknown *aCallerState)
{
  ...
  ReentrantMonitorAutoEnter mon(mReentrantMonitor);
  WMF_LOG("[%p] WMFByteStream::BeginRead() mOffset=%lld tell=%lld length=%lu mIsShutdown=%d",
                                  ...
                                  ReadRequest* aRequestState)
{
  if (mResource->GetLength() > -1 &&
    ...
    WMF_LOG("[%p] WMFByteStream::ProcessReadRequest() read offset greater than length, soft-failing read", this);
    return;
  }
  ...
  WMF_LOG("[%p] WMFByteStream::ProcessReadRequest() read %d at %lld finished rv=%x",
...
WMFByteStream::BeginWrite(const BYTE *, ULONG ,
                          ...
                          IUnknown *)
{
  WMF_LOG("[%p] WMFByteStream::BeginWrite()", this);
  return E_NOTIMPL;
}
...
WMFByteStream::Close()
{
  WMF_LOG("[%p] WMFByteStream::Close()", this);
  return S_OK;
}
...
WMFByteStream::EndRead(IMFAsyncResult* aResult, ULONG *aBytesRead)
{
  ...
  *aBytesRead = requestState->mBytesRead;
  ...
  WMF_LOG("[%p] WMFByteStream::EndRead() offset=%lld *aBytesRead=%u mOffset=%lld status=0x%x hr=0x%x eof=%d",
...
{
  WMF_LOG("[%p] WMFByteStream::EndWrite()", this);
  return E_NOTIMPL;
}
...
WMFByteStream::Flush()
{
  WMF_LOG("[%p] WMFByteStream::Flush()", this);
  return S_OK;
}
...
{
  WMF_LOG("[%p] WMFByteStream::GetCapabilities()", this);
...
WMFByteStream::GetCurrentPosition(QWORD *aPosition)
{
  ...
  *aPosition = mOffset < 0 ? mResource->GetLength() : mOffset;
  WMF_LOG("[%p] WMFByteStream::GetCurrentPosition() %lld", this, mOffset);
  return S_OK;
}
...
WMFByteStream::GetLength(QWORD *aLength)
{
  ...
  *aLength = mResource->GetLength();
  WMF_LOG("[%p] WMFByteStream::GetLength() %lld", this, *aLength);
  return S_OK;
}
...
WMFByteStream::IsEndOfStream(BOOL *aEndOfStream)
{
  ...
  *aEndOfStream = IsEOS();
  WMF_LOG("[%p] WMFByteStream::IsEndOfStream() %d", this, *aEndOfStream);
  return S_OK;
}
...
WMFByteStream::Read(BYTE* aBuffer, ULONG aBufferLength, ULONG* aOutBytesRead)
{
  ...
  if (NS_FAILED(Read(&request))) {
    WMF_LOG("[%p] WMFByteStream::Read() offset=%lld failed!", this, mOffset);
    return E_FAIL;
  }
  if (aOutBytesRead) {
    *aOutBytesRead = request.mBytesRead;
  }
  WMF_LOG("[%p] WMFByteStream::Read() offset=%lld length=%u bytesRead=%u",
...
{
  WMF_LOG("[%p] WMFByteStream::Seek(%d, %lld)", this, aSeekOrigin, aSeekOffset);
...
{
  ReentrantMonitorAutoEnter mon(mReentrantMonitor);
  WMF_LOG("[%p] WMFByteStream::SetCurrentPosition(%lld)",
          this, aPosition);
...
{
  WMF_LOG("[%p] WMFByteStream::SetLength()", this);
  return E_NOTIMPL;
}
...
{
  WMF_LOG("[%p] WMFByteStream::Write()", this);
  return E_NOTIMPL;
}
