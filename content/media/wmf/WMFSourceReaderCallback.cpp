...
{
  WMF_READER_LOG("WMFSourceReaderCallback::QueryInterface %s", GetGUIDName(aIId).get());
...
WMFSourceReaderCallback::OnReadSample(HRESULT aReadStatus,
                                      ...
                                      IMFSample *aSample)
{
  WMF_READER_LOG("WMFSourceReaderCallback::OnReadSample() hr=0x%x flags=0x%x time=%lld sample=%p",
...
{
  WMF_READER_LOG("WMFSourceReaderCallback::Cancel()");
...
{
  ReentrantMonitorAutoEnter mon(mMonitor);
  WMF_READER_LOG("WMFSourceReaderCallback::Wait() starting wait");
  while (!mReadFinished) {
    mon.Wait();
  }
  mReadFinished = false;
  WMF_READER_LOG("WMFSourceReaderCallback::Wait() done waiting");
